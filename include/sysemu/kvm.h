...
/**
 * Returns the file descriptor of /dev/kvm-vgpu.
 */
int kvm_get_kvm_vgpu_fd(KVMState *kvm_state);

/**
 * Returns the ID associated with the current VM.
 */
int kvm_get_vm_id(KVMState *kvm_state);

/**
 * Returns the base address (mmap'd in QEMU's address space) of the
 * hypervisor-managed shared memory buffer.
 */
void *kvm_get_shm_base(KVMState *kvm_state);

/**
 * The APIs notify the KVM hypervisor of the VM states via IOCTL.
 */
...
/**
 * The APIs request zero copy region information.
 */
