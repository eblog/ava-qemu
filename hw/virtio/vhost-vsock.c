...
static void vhost_vsock_device_realize(DeviceState *dev, Error **errp)
{
    ...
    /* Notify hypervisor of the guest-cid */
    printf("notify hypervisor of guest-cid (%ld)\n", vsock->conf.guest_cid);
    ret = ioctl(kvm_get_kvm_vgpu_fd(kvm_state), KVM_SET_VM_GUEST_CID, vsock->conf.guest_cid);
    if (ret < 0) {
        error_setg_errno(errp, -ret, "vhost-vsock: unable to notify guest cid");
        goto err_vhost_dev;
    }
#endif
